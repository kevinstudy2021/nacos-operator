...
import (
	"fmt"
	"github.com/kevinstudy2021/nacosOperator/pkg/util/merge"
...
type IKindClient interface {
	Ensure(nacos middlewarev1alpha1.Nacos)
	EnsureStatefulset(nacos middlewarev1alpha1.Nacos)
	EnsureConfigmap(nacos middlewarev1alpha1.Nacos)
}
...
func (e *KindClient) generateName(nacos *middlewarev1alpha1.Nacos) string {
	return nacos.Name
}

func (e *KindClient) generateHeadlessSvcName(nacos *middlewarev1alpha1.Nacos) string {
	return fmt.Sprintf("%s-headless", nacos.Name)
}
func (e *KindClient) generateClientSvcName(nacos *middlewarev1alpha1.Nacos) string {
	return fmt.Sprintf("%s-client", nacos.Name)
}

// CR格式验证
func (e *KindClient) ValidationField(nacos *middlewarev1alpha1.Nacos) {

	setDefaultValue := []func(nacos *middlewarev1alpha1.Nacos){
		setDefaultNacosType,
		setDefaultMysql,
...
func setDefaultNacosType(nacos *middlewarev1alpha1.Nacos) {
	// 默认设置单节点
	if nacos.Spec.Type == "" {
...
func setDefaultCertification(nacos *middlewarev1alpha1.Nacos) {
	// 默认设置认证参数
...
func setDefaultMysql(nacos *middlewarev1alpha1.Nacos) {
	// 默认设置内置数据库
...
func (e *KindClient) EnsureStatefulsetCluster(nacos *middlewarev1alpha1.Nacos) {
...
func (e *KindClient) EnsureStatefulset(nacos *middlewarev1alpha1.Nacos) {
	ss := e.buildStatefulset(nacos)
	ss.Spec.Template.Spec = merge.PodSpec(ss.Spec.Template.Spec, nacos.Spec.K8sWrapper.PodSpec.Spec)
	myErrors.EnsureNormal(e.k8sService.CreateOrUpdateStatefulSet(nacos.Namespace, ss))
}

func (e *KindClient) EnsureService(nacos *middlewarev1alpha1.Nacos) {
	ss := e.buildService(nacos)
	myErrors.EnsureNormal(e.k8sService.CreateIfNotExistsService(nacos.Namespace, ss))
}

func (e *KindClient) EnsureServiceCluster(nacos *middlewarev1alpha1.Nacos) {
	ss := e.buildService(nacos)
	myErrors.EnsureNormal(e.k8sService.CreateOrUpdateService(nacos.Namespace, ss))
}

func (e *KindClient) EnsureClientService(nacos *middlewarev1alpha1.Nacos) {
	ss := e.buildClientService(nacos)
	myErrors.EnsureNormal(e.k8sService.CreateIfNotExistsService(nacos.Namespace, ss))
}

func (e *KindClient) EnsureHeadlessServiceCluster(nacos *middlewarev1alpha1.Nacos) {
	ss := e.buildService(nacos)
	ss = e.buildHeadlessServiceCluster(ss, nacos)
	myErrors.EnsureNormal(e.k8sService.CreateOrUpdateService(nacos.Namespace, ss))
}

func (e *KindClient) EnsureConfigmap(nacos *middlewarev1alpha1.Nacos) {
...
func (e *KindClient) EnsureMysqlConfigMap(nacos *middlewarev1alpha1.Nacos) {
	cm := e.buildMysqlConfigMap(nacos)
	myErrors.EnsureNormal(e.k8sService.CreateIfNotExistsConfigMap(nacos.Namespace, cm))
}

func (e *KindClient) EnsureJob(nacos *middlewarev1alpha1.Nacos) {
	// 使用job执行SQL脚本的逻辑
...
func (e *KindClient) buildMysqlConfigMap(nacos *middlewarev1alpha1.Nacos) *v1.ConfigMap {
...
func (e *KindClient) buildJob(nacos *middlewarev1alpha1.Nacos) *batchv1.Job {
	labels := e.generateLabels(nacos.Name, NACOS)
	labels = e.MergeLabels(nacos.Labels, labels)
...
func (e *KindClient) buildService(nacos *middlewarev1alpha1.Nacos) *v1.Service {
	labels := e.generateLabels(nacos.Name, NACOS)
	labels = e.MergeLabels(nacos.Labels, labels)
...
func (e *KindClient) buildClientService(nacos *middlewarev1alpha1.Nacos) *v1.Service {
	labels := e.generateLabels(nacos.Name, NACOS)
	labels = e.MergeLabels(nacos.Labels, labels)
...
func (e *KindClient) buildStatefulset(nacos *middlewarev1alpha1.Nacos) *appv1.StatefulSet {
	// 生成label
	labels := e.generateLabels(nacos.Name, NACOS)
	// 合并cr中原有的label
	labels = e.MergeLabels(nacos.Labels, labels)
...
func (e *KindClient) AddCheckDatabase(nacos *middlewarev1alpha1.Nacos, sts *appv1.StatefulSet) *appv1.StatefulSet {
...
func (e *KindClient) buildConfigMap(nacos *middlewarev1alpha1.Nacos) *v1.ConfigMap {
	labels := e.generateLabels(nacos.Name, NACOS)
	labels = e.MergeLabels(nacos.Labels, labels)
...
func (e *KindClient) buildDefaultConfigMap(nacos *middlewarev1alpha1.Nacos) *v1.ConfigMap {
	labels := e.generateLabels(nacos.Name, NACOS)
	labels = e.MergeLabels(nacos.Labels, labels)
	data := make(map[string]string)
...
func (e *KindClient) buildStatefulsetCluster(nacos *middlewarev1alpha1.Nacos, ss *appv1.StatefulSet) *appv1.StatefulSet {
...
func (e *KindClient) buildHeadlessServiceCluster(svc *v1.Service, nacos *middlewarev1alpha1.Nacos) *v1.Service {
