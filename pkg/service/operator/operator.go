...
func (c *OperatorClient) MakeEnsure(nacos *middlewarev1alpha1.Nacos) {
	// 验证CR字段
	...
	default:
		panic(any(myErrors.New(myErrors.CODE_PARAMETER_ERROR, myErrors.MSG_PARAMETER_ERROT, "nacos.Spec.Type", nacos.Spec.Type)))
	}
}

func (c *OperatorClient) PreCheck(nacos *middlewarev1alpha1.Nacos) {
	switch nacos.Status.Phase {
	case middlewarev1alpha1.PhaseFailed:
		// 失败，需要修复
		c.HealClient.MakeHeal(nacos)
	case middlewarev1alpha1.PhaseNone:
		// 初始化
		nacos.Status.Phase = middlewarev1alpha1.PhaseCreating
		panic(any(myErrors.New(myErrors.CODE_NORMAL, "")))
	case middlewarev1alpha1.PhaseScale:
	default:
		// TODO
	}
}

func (c *OperatorClient) CheckAndMakeHeal(nacos *middlewarev1alpha1.Nacos) {
	// 检查kind
...
func (c *OperatorClient) UpdateStatus(nacos *middlewarev1alpha1.Nacos) {
