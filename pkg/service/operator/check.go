...
import (
	...
	nacosClient "github.com/kevinstudy2021/nacosOperator/pkg/service/nacos"
)
...
func (c *CheckClient) CheckKind(nacos *nacosgroupv1alpha1.Nacos) []corev1.Pod {
	...
	if *ss.Spec.Replicas != *nacos.Spec.Replicas {
		panic(any(myErrors.New(myErrors.CODE_ERR_UNKNOW, "cr replicas is not equal ss replicas")))
	...
	if len(pods) < (int(*nacos.Spec.Replicas)+1)/2 {
		panic(any(myErrors.New(myErrors.CODE_ERR_UNKNOW, "The number of ready pods is too less")))
